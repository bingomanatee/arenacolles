...
/// <summary>In this example we adjust the grid's rendering range at runtime to the view rectangle or the main camera.</summary>
/// 
/// Rendering a very large amount of grid lines can get expensive, and when the player will only ever see 
/// just a small portion of the grid it's a complete waste of resources. We will instead fake the illustion
/// of an infinitely large grid by dynamically adjusting the rendering range as the camera is moved.
/// 
/// The view rectangle of an orthographic camera is defined by the camera's orthographic size and its aspect
/// ratio. The orthographic size is the height of the rectangle in world units and the width is comuted by
/// multiplying the height with the aspect ratio.
/// 
/// <code>width = height * aspectRatio</code>
/// 
/// The postion of the centre of the rectangle is the position of the camera in the world. We need the lower
/// left-hand and upper righ-hand corners of the rectangle as the rendering range of out grid.
/// 
/// We will go slightly beyond the range of the camera for optimisation purposed. Every time the range is 
/// changed the grid lines need to be re-calculated. By going slightly beyond the view rectangle we give the
/// camera an are of tolerance where it can move without triggering re-calculation.
/// 
/// Reading topics of interest:
/// <a href="http://docs.unity3d.com/ScriptReference/Camera-orthographicSize.html">Camera.orthographicSize</a>
